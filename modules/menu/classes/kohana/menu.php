...
class Kohana_Menu
{
	 ...
	 */
	public function __construct($config, $menu_config = array())
	{
		...
		$this->view = View::factory($this->config['view']);
                ...
                if ($this->config['driver'] == 'database') {
                    ...
                    $menu->where('hidden', '=', 0);
                    $menu->where('menu_id', '=', $menu_config['id']);
                    $menu->order_by('position');
                    $items = $menu->where('parent_id', '=', 0)->find_all();
                    
                    $this->menu['items'] = $this->get_from_database_orm($items, $menu_config);
                }
	 ...
	 * @return	Menu
	 */
	public static function factory($config = 'default', $menu_config = array())
	{
		return new Menu('menu/'.$config, $menu_config);
	}
	...
	public function render()
	{
         ...
         */
        private function get_from_database_orm($items, $menu_config = array()) {
            $temp = array();
            foreach ($items as $key => $item) {
                $real_url = unserialize($item->data);
                $temp[$key]['url'] = $real_url['url'];
                $temp[$key]['title'] = $item->title;
                $temp[$key]['parent_id'] = $item->parent_id;
                $temp[$key]['menu_config'] = $menu_config;
                if ($item->classes) {
                        $temp[$key]['classes'] = array($item->classes);
                }
                if ($item->icons) {
                        $temp[$key]['icons'] = array($item->icons);
                }
                
                $subcategories = $item->subcategories->find_all();
                if ($subcategories->count() > 0)
                    $temp[$key]['items'] = $this->get_from_database_orm($subcategories, $menu_config);
            }
            return $temp;
        }
