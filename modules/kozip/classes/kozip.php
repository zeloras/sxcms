 ...
 *
 * This class is based on a library I found in CodeIgniter:
 * http://codeigniter.com/user_guide/libraries/zip.html
 *
 * The original library wasn't chainable which I found annoying
 * so I added it -- Flynsarmy
 *
 * @package		Kohana
 * @subpackage	Modules
 * @category	Encryption
 * @author		Flynsarmy and ExpressionEngine Dev Team
...
class KOZip  {
	...
	public function __construct()
	{
		...
		$this->Log->add(Log::INFO, "Zip Compression Class Initialized");
	...
	function read_file($path, $preserve_filepath = FALSE)
	{
		if ( ! file_exists($path))
		{
			throw new Exception('KOZip: File does not exist.');
			return $this;
		}
		...
		throw new Exception('KOZip: Could not read from file.');
		return $this;
	}
	...
	function read_dir($path, $preserve_filepath = TRUE, $root_path = NULL)
	{
		if ( ! $fp = @opendir($path))
		{
			throw new Exception('KOZip: Could not open directory.');
			return $this;
		}
	...
	function archive($filepath)
	{
		...
		{
			throw new Exception('KOZip: Could not open output file.');
			return $this;
		}
	 ...
	 * @return	bool
	 */
	function download(Response $Response, $filename = 'backup.zip')
	{
		...
		$Response
			...
			->headers("Content-Length", ''.$this->offset)
			//->send_headers()
			->body( $zip_content );
	}
