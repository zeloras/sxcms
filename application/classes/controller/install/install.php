...
class Controller_Install_Install {
    ...
    public function step1()
    {
        $data['settings'] = array(
            ...
            'per_config' => (substr(decoct(fileperms(DOCROOT.'config.xml')),3) == 644),
        );
    ...
    public function step2()
    {
        ...
        else
        {
            ...
            mysql_connect($_POST['mysql']['server'], $_POST['mysql']['login'], $_POST['mysql']['password']);
            mysql_select_db($_POST['mysql']['database']);
            mysql_query("SET NAMES `utf8`");
            $show_tables = mysql_query("SHOW TABLES FROM ".$_POST['mysql']['database']);
            while ($rows = mysql_fetch_array($show_tables))
                if (in_array($rows[0], $system_tables))
                    $sql_drop_isset_table = mysql_query("DROP TABLE `".$rows[0]."`");
            ...
            $add_user = mysql_query("INSERT INTO sx_users (`email`, `password`, `username`) VALUES ('".$_POST['user']['email']."', '".$hash_password."', '".$user_name."')");
            $add_role_user = mysql_query("INSERT INTO `sx_roles_users` (`user_id`, `role_id`) VALUES ('1', '2')");
